...
class MultisaleProducts extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocListener<MultipleSaleBloc, MultipleSaleState>(
      listener: (context, state) {
        if (state is MultipleSaleSuccessState) {
          context.pushNamed(
            AppRoutes.getCleanRouteName(AppRoutes.multipleSaleSuccessPage),
          );
        } else if (state is MultipleSaleErrorState) {
          showCustomSnackBar(
            context: context,
            bannerType: BannerTypes.errorBanner.type,
            bannerMessage: AppLocalizations.of(
              context,
            )!.error_membership_payment,
          );
        }
      },
      child: PopScope(
