...
import 'package:smart_lunch/core/base_widgets/cards/product_permission_card.dart';
...
class ProhibitedProductsPage extends StatefulWidget {
  const ProhibitedProductsPage({super.key});

  @override
  State<ProhibitedProductsPage> createState() => _ProhibitedProductsPageState();
}

class _ProhibitedProductsPageState extends State<ProhibitedProductsPage> {
  int? _studentId;
    ...
    return BlocBuilder<ProductRestrictionBloc, ProductRestrictionState>(
      builder: (context, state) {
        ...
        final List<ProductRestriction> restrictions =
            state is ProductRestrictionLoaded
            ? state.restrictions
        ...
        return ListView.separated(
          ...
          itemBuilder: (context, index) {
            ...
            final isRestricted = restrictions.any(
              (r) =>
                  r.student == studentId &&
                  r.product == product.id &&
                  r.restrictionType == "prohibited",
            );

            return ProductPermissionCard(
              image: (product.imageUrl ?? '').isNotEmpty
                  ? NetworkImage(product.imageUrl!)
                  : AssetImage(AppImages.defaultProductImage) as ImageProvider,
